...
func speedtest1_final(tls *libc.TLS) {
	...
	if g.bVerify != 0 {
		...
		HashFinal(tls)
		for i = 0; i < 24; i++ {
			libc.Xprintf(tls, ts+3056, libc.VaList(bp+40, int32(*(*uint8)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 3116 + 259 + uintptr(i))))))
		}
			...
			if g.bVerify != 0 {
				...
				*(*uint8)(unsafe.Pointer(bp + 8 + 1)) = *(*uint8)(unsafe.Pointer(ts + 3163 + uintptr(eType)))
				...
				if eType == SQLITE_FLOAT {
					*(*u64)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 96)) += uint64(2)
					...
					*(*u64)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 96)) += u64(nBlob*2 + 2)
				} else {
					HashUpdate(tls, z, uint32(len))
					*(*u64)(unsafe.Pointer(uintptr(unsafe.Pointer(&g)) + 96)) += u64(len + 2)
				}
			}
